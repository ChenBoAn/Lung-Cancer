...
def label_nodule(original_path, mask_path, nodule_path, coordinate, hu):
    ...
    #? binary = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 7, 5)
    #? binary = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 9, 5)
...
def main():
    ...
    for patient_id in range(1, 201):
        ...
        if os.path.isdir("G:/Hospital_data/nodule/partial/mask/" + str(patient_id).zfill(3) + "/"):
            ...
            nodule_path = "E:/Lung_Cancer/Paper/Compare_Dynamic_Threshold/OTSU/" + str(patient_id) + "/"
            ...
            for i in range(len(nodules_coordinate)):
                ...
                label_nodule(image_path, mask_path, nodule_num_path, nodules_coordinate[i], patient_hu[nodules_coordinate[i][2] - 1])
    ...
    with pd.ExcelWriter(engine='openpyxl', path='E:/Lung_Cancer/Paper/Compare_Dynamic_Threshold/Total_Compare_180.xlsx', mode='a') as writer:
        df.to_excel(writer, sheet_name='OTSU', index=False)
