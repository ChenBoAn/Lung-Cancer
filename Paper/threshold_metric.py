...
for n in range(1, 7):
    ...
    predict_path = "E:/Lung_Cancer/Lung_Nodule_Segmentation/final3/2/threshold" + str(n) + "/"
    answer_path = "G:/Hospital_data/nodule/final/mask/"
    for i in os.listdir(answer_path):
        answer_patient_path = answer_path + i + "/"
        predict_patient_path = predict_path + str(int(i)) + "/"
        ...
        for j in os.listdir(answer_patient_path):
            ...
            for k in os.listdir(answer_nodule_path):
                ...
                predict = cv2.imread(predict_nodule_path + str(int(k[:-4]) + 1).zfill(4) + '.tif', 0)
    ...
    with pd.ExcelWriter(engine='openpyxl', path='E:/Lung_Cancer/Lung_Nodule_Segmentation/final3/2/result/result.xlsx', mode='a') as writer:
