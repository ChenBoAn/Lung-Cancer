...
def add_zero(total_path, partial_path):
    ...
    for i in os.listdir(total_image_path):
        image_path = os.path.join(total_image_path, i)

        mask_save_path = os.path.join(total_mask_path, i)
        if not os.path.isdir(mask_save_path): os.mkdir(mask_save_path)
        
        for j in os.listdir(image_path):
            image_path = os.path.join(image_path, j)
            mask_path = partial_mask_path + i + '/' + j
            #* 無mask時，填補全為0的mask
            if not os.path.isfile(mask_path):
                mask = np.zeros((512, 512), np.uint8)
                cv2.imwrite(total_mask_path + i + '/' + j, mask)
            else:
                shutil.copyfile(mask_path, total_mask_path + i + '/' + j) # partial_mask_path -> total_mask_path
...
#? check_num(partial_path)
#? copy_picture(final_path + 'mask/', partial_path + 'mask/')
